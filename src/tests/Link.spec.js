...
describe('<Link />', () => {
  ...
  beforeEach(() => {
    RouteFactoryStub = sinon.stub(RouteFactory, 'parse').callsFake((to) => {
      switch (to) {
        case '/about':
  ...
  it('renders successfully', () => {
    const wrapper = shallow(<div><Link navigate={() => true} routeName={'myRoute'} to={'/about'}>link</Link></div>);
  ...
  it('implements default base class', () => {
    const wrapper = shallow(<Link navigate={() => true} routeName={'myRoute'} to={'/about'}>link</Link>);
    assert(wrapper.node.props.className.includes('Link'));
  });

  it('implements className', () => {
    const myClass = 'myClass';
    const wrapper = shallow(<Link navigate={() => true} className={myClass} routeName={'myRoute'} to={'/about'}>link</Link>);
    assert(wrapper.node.props.className.includes(myClass));
  });

  it('implements default active class', () => {
    const wrapper = shallow(<Link navigate={() => true} routeName={'root.about'} to={'/about'}>link</Link>);
  ...
  it('implements custom base class active class', () => {
    ...
    const wrapper = shallow(<Link navigate={() => true} baseClass={baseClass} routeName={'root.about'} to={'/about'}>link</Link>);
    assert(wrapper.node.props.className.includes(`${baseClass}--active`));
  });

  it('calls navigate when clicked', () => {
    const onNavigate = sinon.spy();
    const wrapper = shallow(<Link navigate={onNavigate} routeName={'root.about'} to={'/about'}>link</Link>);
